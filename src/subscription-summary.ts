...
export class MonthlyGivingCircle extends LitElement {
              ...
              <li>
                <div class="info">
                  ...
                  <div class="next-donation">
                    ...
                    <p>
                      ${nextBillingDate
                        ? this.formatTime(nextBillingDate)
                        : 'not found'}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  @click=${() => this.manageDonation(plan)}
                >
  ...
  formatTime(
    nextBillingDateInfo: SubscriptionSummary['payment']['nextBillingDate']
  ): string {
    if (!nextBillingDateInfo) {
      return '';
    }
    const time = new Date(nextBillingDateInfo.date);
    const timeToDisplay = time.toLocaleDateString('default', {
      month: 'long',
      day: 'numeric',
      year: 'numeric',
    });
  ...
  manageDonation(plan: SubscriptionSummary) {
    ...
    this.dispatchEvent(
      new CustomEvent('displaySubscriptionDetails', {
        detail: { plan },
        bubbles: true,
        composed: true,
      })
