...
import type { MGCEditSubscription } from './edit-subscription';
...
export class MonthlyGivingCircle extends LitElement {
  ...
  @property({ type: Array }) subscriptionUpdatedMessages: UpdatedMessage[] = [];

  @query('iaux-mgc-edit-subscription') editSubEl!: MGCEditSubscription;
  ...
  public createSummary(plan: any): SubscriptionSummary {
    console.log('--- planUpdateComplete', plan);
    const summary = new SubscriptionSummary(plan);
    return summary;
  }

  // close form
  // empty form innards
  public subscriptionUpdated(updateDetails: UpdatedMessage) {
    const { updatedSection } = updateDetails;
    const updatedMessageList = this.subscriptionUpdatedMessages;
    updatedMessageList.push(updateDetails);
    this.subscriptionUpdatedMessages = updatedMessageList;

    // if (updatedSection === 'edit') {}
    this.editSubEl.clearForm(updatedSection as string);
    console.log('subscriptionUpdated', updateDetails);
  }

  get certainPlan(): SubscriptionSummary | undefined {
    const certainPlan = this.activePlans.find(
      plan => plan.id === this.planIdToDisplay
    );
    console.log('certain plan --- ', this.planIdToDisplay, certainPlan);
    return certainPlan;
  }
  ...
  protected render() {
      ...
      return html`
        <iaux-mgc-recent-donations
          ...
          @EmailReceiptRequest=${(e: CustomEvent) => {
            ...
            this.dispatchEvent(
              new CustomEvent('sendDonationReceipt', { detail: { donation } })
            );
          }}
        ></iaux-mgc-recent-donations>
      `;
    ...
    const withPlansClass = this.activePlans.length
      ? 'with-plans'
      : 'without-plans';
    return html`
      <section class=${withPlansClass}>
        ...
        <div class=${`subscription-updates`}>
          ${this.subscriptionUpdatedMessages.map(
            (message: UpdatedMessage) => html`
              <div
      ...
      return html`
        <iaux-mgc-edit-subscription
          ...
          @editDate=${(e: CustomEvent) => {
            console.log('updateDate mgc!', e.detail);
            const { plan } = e.detail;
            const { newDate } = e.detail;

            this.dispatchEvent(
              new CustomEvent('editDate', {
                detail: {
                  plan,
                  newDate,
                },
          ...
          @cancelPlan=${(e: CustomEvent) => {
            console.log('cancelPlan mgc!', e.detail);
            const { plan } = e.detail;
      ...
      return html`
        <iaux-mgc-subscription-summary
          ...
          @displaySubscriptionDetails=${(e: CustomEvent) => {
            ...
            debugger;
            // get token and set planIdToDisplay
