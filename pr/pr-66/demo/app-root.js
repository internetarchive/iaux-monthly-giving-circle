...
`,V;(function(e){e.Loading=`loading`,e.Available=`available`,e.Unavailable=`unavailable`})(V||={});var H=class extends x{constructor(){super(...arguments),this.donationInfoValid=!0,this.applePayMode=V.Loading,this.googlePayMode=V.Loading,this.venmoMode=V.Loading,this.payPalMode=V.Loading,this.paymentModeSelected=void 0}render(){let e=this.paymentModeSelected?`payment-selected`:``;return y`
  ...
  `,Kr=u([S(`ia-mgc-form-section-info`)],Kr);var qr=class extends x{constructor(){super(...arguments),this.status=``}get glyph(){return this.status===`success`?`✓`:this.status===`fail`?`✖`:``}render(){return this.status?y`
  ...
  `,u([C({type:Object})],K.prototype,`plan`,void 0),u([C({type:String})],K.prototype,`newDate`,void 0),u([C({type:Boolean,reflect:!0})],K.prototype,`currentlyEditing`,void 0),u([C({type:Boolean})],K.prototype,`allowEditing`,void 0),u([C({type:String})],K.prototype,`updateMessage`,void 0),u([C({type:String})],K.prototype,`errorMessage`,void 0),u([C({type:String})],K.prototype,`warningMessage`,void 0),u([C({type:String})],K.prototype,`updateStatus`,void 0),u([w(`form`)],K.prototype,`form`,void 0),u([w(`form ia-mgc-button#edit-date`)],K.prototype,`formSubmitButton`,void 0),u([w(`form input[name="edit-date"]`)],K.prototype,`dateInput`,void 0),K=u([S(`ia-mgc-edit-date`)],K);var Yr=class extends x{constructor(){super(...arguments),this.patronWantsToKeepPlan=!0,this.initialCancelRequest=!1}updated(e){e.has(`plan`)&&console.log(`plan updated`,this.plan)}async cancelThisPlan(e){e.preventDefault(),this.patronWantsToKeepPlan=!1,this.dispatchEvent(new Event(`cancelPlan`))}get formId(){return`cancel-donation-form-${this.plan?.id}`}render(){return this.initialCancelRequest?this.confirmCancelation:y`
      <donation-form-section
        badgemode="hidebadge"
  ...
  `,u([C({type:Object})],Yr.prototype,`plan`,void 0),u([C({type:Boolean,reflect:!0})],Yr.prototype,`patronWantsToKeepPlan`,void 0),u([C({type:Boolean,reflect:!0})],Yr.prototype,`initialCancelRequest`,void 0),u([w(`form`)],Yr.prototype,`form`,void 0),Yr=u([S(`ia-mgc-cancel-plan`)],Yr);var q=class extends x{constructor(){super(...arguments),this.displayCreditCard=!1,this.patronEmail=``,this.elementConnected=!1}get braintreeInputs(){return{errorMessage:this.querySelector(`#braintree-error-message`),number:this.querySelector(`#braintree-creditcard`),cvv:this.querySelector(`#braintree-cvv`),expirationDate:this.querySelector(`#braintree-expiration`)}}createRenderRoot(){return this}disconnectedCallback(){this.elementConnected=!1}connectedCallback(){super.connectedCallback(),console.log(`connectedCallback`,{paymentConfig:this.paymentConfig}),this.elementConnected=!0}updated(e){if(e.has(`elementConnected`)&&this.elementConnected&&this.paymentConfig){let{braintreeAuthToken:e,endpointManager:t,paymentClients:n,environment:r}=this.paymentConfig||{};!this.braintreeManager&&e&&t&&n&&r&&this.setupBraintreeManager()}this.braintreeManager&&e.has(`displayCreditCard`)&&this.displayCreditCard&&this.setupCreditCardHandler()}async validateCreditCardFields(){let e=await this.braintreeManager?.paymentProviders.creditCardHandler.get(),t=!1,n=null;try{n=await e?.tokenizeHostedFields(),t=!0}catch(t){let n=t;switch(e?.showErrorMessage(),n.code){case`HOSTED_FIELDS_FIELDS_EMPTY`:e?.markFieldErrors([M.Number,M.CVV,M.ExpirationDate]);break;case`HOSTED_FIELDS_FIELDS_INVALID`:break;case`HOSTED_FIELDS_TOKENIZATION_FAIL_ON_DUPLICATE`:break;case`HOSTED_FIELDS_TOKENIZATION_CVV_VERIFICATION_FAILED`:e?.markFieldErrors([M.CVV]);break;case`HOSTED_FIELDS_FAILED_TOKENIZATION`:break;case`HOSTED_FIELDS_TOKENIZATION_NETWORK_ERROR`:break;default:break}}return t?n:!1}async setupCreditCardHandler(){let e=await this.braintreeManager?.paymentProviders.creditCardHandler.get();try{await e?.tokenizeHostedFields()}catch{}e?.removeFieldErrors([M.Number,M.CVV,M.ExpirationDate])}render(){return y` <div>${this.creditCardTemplate}</div> `}lightDomCSS(){return g`
        ...
        ${this.currentlyEditing?y`
            <form id="edit-plan-payment-method" @submit=${this.submitPaymentMethodChange}>
              ...
              <ia-mgc-button
                ...
                .clickHandler=${async(e,t)=>{console.log({iaButton:t,event:e});let n=t;if(n.isDisabled=!0,!this.creditCardElement?.reportValidity()){n.isDisabled=!1;return}let r=await this.braintreeManagerElement?.validateCreditCardFields();if(!r){n.isDisabled=!1;return}let i=new Xr({paymentMethodInfo:r,donorContactInfo:this.contactFormElement?.donorContactInfo,paymentProvider:this.selectedPaymentProvider});this.dispatchEvent(new CustomEvent(`UpdatePaymentMethod`,{detail:{newPaymentMethodRequest:i}}))}}
    ...
    `}get sectionTitle(){let e=``,t=``,n=y``,r=y`
      <ia-mgc-button
        class="link slim"
        .clickHandler=${async()=>{this.viewToDisplay=`receipts`,await this.updateComplete,this.dispatchEvent(new Event(Zr.receipts))}}
      >
    ...
    `,i=this.receipts.length&&(this.viewToDisplay===`plans`||this.viewToDisplay===`welcome`);switch(this.viewToDisplay){case`receipts`:e=`Recent donations`,t=`default`,n=y`<ia-mgc-button
          class="primary"
          id="close-receipts"
          .clickHandler=${async()=>{this.viewToDisplay=this.plans.length?`plans`:`welcome`;let e=this.plans.length?Zr.plans:Zr.welcome;this.dispatchEvent(new Event(e)),this.updates=[],await this.updateComplete}}
        >
    ...
    `}get nonEditView(){return this.viewToDisplay===`receipts`?y`
      ...
      ${this.viewToDisplay===`plans`&&this.plans.length?y`
            <ia-mgc-plans
              @editThisPlan=${async e=>{this.editingThisPlan=e.detail.plan,this.viewToDisplay=`editPlan`,this.dispatchEvent(new Event(Zr.editPlan)),await this.updateComplete}}
              .plans=${this.plans}
          ...
          ></ia-mgc-welcome>`}
    `}};u([C({type:String})],X.prototype,`patronName`,void 0),u([C({type:String})],X.prototype,`patronEmail`,void 0),u([C({type:Boolean})],X.prototype,`canEditPaymentMethod`,void 0),u([C({type:Array})],X.prototype,`receipts`,void 0),u([C({type:Array})],X.prototype,`updates`,void 0),u([C({type:Array})],X.prototype,`plans`,void 0),u([C({type:Object})],X.prototype,`editingThisPlan`,void 0),u([C({type:String,reflect:!0})],X.prototype,`viewToDisplay`,void 0),u([C({type:Boolean,reflect:!0})],X.prototype,`canEdit`,void 0),u([C({type:Object})],X.prototype,`paymentConfig`,void 0),X=u([S(`ia-monthly-giving-circle`)],X);var Qr=class{constructor(e){this.receipt=e}get amount(){return Gr(this.receipt.total_amount,this.receipt.currency??`USD`)}get isTest(){return this.receipt.isTest??!1}get id(){return this.receipt.token??`no token found`}get date(){return this.receipt.date??`no date found`}},$r=class{constructor(e){this.plan=e,this.currency=e.currency??`USD`}get id(){return this.plan.token}get amount(){return this.plan.amount}get amountFormatted(){return Gr(this.plan.amount,this.currency)}get payment(){return this.plan.btdata}setAmount(e){this.plan.oldAmount=this.plan.amount,this.plan.amount=e}get nextBillingDate(){return this.payment?.nextBillingDate?.date??``}setNextBillingDate(e){this.payment&&(this.payment.nextBillingDate.oldDate=this.payment.nextBillingDate.date,this.payment.nextBillingDate.date=e)}formatDateUTC(e){let t=new Date(e);return Number.isNaN(t.getTime())?`Invalid date`:new Intl.DateTimeFormat(`en-US`,{year:`numeric`,month:`short`,day:`numeric`,timeZone:`UTC`}).format(t)}get nextBillingDateLocale(){let e=this.payment?.nextBillingDate.date??``;return e?this.formatDateUTC(e):`not found`}get lastBillingDateLocale(){let e=this.payment?.lastBillingDate.date??``;return e?this.formatDateUTC(e):``}get hasBeenCancelled(){return this.plan.isCancelled===!0}get isTest(){return this.plan.is_test}cancelPlan(){this.plan.isCancelled=!0}setNewProcessorId(e){let t=this.plan.processor_id;this.plan.processor_id=e,this.plan.oldProcessorId=t}setNewPaymentMethod(e){let t=this.payment,n={...this.plan.btdata,...e.paymentMethodInfo.details,last4:e.paymentMethodInfo.details.lastFour??`unknown`};this.plan.old_btData=t,this.plan.btdata=n}},Z=[],ei=new Date;ei.setDate(ei.getDate()+25),ei.setMinutes(0),ei.setSeconds(0),ei.setMilliseconds(0),console.log(`nextPaymentIn25Days`,ei.toISOString());var ti=new Date;ti.setMinutes(0),ti.setSeconds(0),ti.setMilliseconds(0),ti.setDate(1),console.log(`lastPaymentInTheSameMonth`,ti.toISOString());var ni=new Date;ni.setMonth(ni.getMonth()+1);var ri=new Date(ni);ri.setMinutes(0),ri.setSeconds(0),ri.setMilliseconds(0),ri.setDate(1),console.log(`nextPaymentFirstOfMonth`,ri.toISOString()),new Date().setHours(0,0,0,0);var ii=new Date;ii.setHours(0,0,0,0),ii.setDate(0);var ai=new Date;ai.setHours(0,0,0,0),ai.setMonth(ai.getMonth()+1),ai.setDate(0);var oi={41:{token:`Acbdcdcadsfdasf.1234alphanumeric.3foobarXyZ`,amount:10.3,start_date:`2024-05-22 00:00:00`,is_test:!0,btdata:{billingDayOfMonth:22,lastBillingDate:{date:ii.toISOString(),timezone_type:3,timezone:`UTC`},nextBillingDate:{date:ai.toISOString(),timezone_type:3,timezone:`UTC`},status:`Active`,paymentMethodType:`Paypal`,last4:null,cardType:null,expirationMonth:null,expirationYear:null,paypalEmail:`donations-buyer@archive.org`}},23764:{token:`Acbdcdcadsfdasf.1234alphanumeric.3foobar`,amount:2.5,start_date:`2022-12-09 00:00:00`,is_test:!0,btdata:{billingDayOfMonth:9,lastBillingDate:{date:ti.toISOString(),timezone_type:3,timezone:`UTC`},nextBillingDate:{date:ri.toISOString(),timezone_type:3,timezone:`UTC`},status:`Active`,paymentMethodType:`creditCard`,last4:`1111`,cardType:`Visa`,expirationMonth:`12`,expirationYear:`2023`}},35406:{token:`Acbdcdcadsfdasf.1234alphanumeric.3foobar444999sparklingCider`,amount:5,currency:`USD`,start_date:`2024-07-01 00:00:00`,is_test:!0,btdata:{billingDayOfMonth:22,nextBillingDate:{date:`2024-08-22 00:00:00.000000`,timezone_type:3,timezone:`UTC`},status:`Active`,paymentMethodType:`Venmo`,last4:null,cardType:null,expirationMonth:null,expirationYear:null,venmoUsername:`venmojoe`}}},si=Object.keys(oi).map(e=>new $r(oi[e])),ci=[new Qr({currency:`USD`,total_amount:1,net_amount:1,fee_amount:0,fee_covered:!1,date:new Date(`2019-01-22 14:26:34`).toLocaleString(`en-US`,{year:`numeric`,month:`short`,day:`numeric`}),donor:`John Doe`,paymentMethod:`Credit Card`,status:`Completed`,id:`foo-id-1`,token:`foo-token-1`,is_test:!1}),new Qr({currency:`USD`,net_amount:9999.99,total_amount:9999.99,fee_amount:0,fee_covered:!1,date:new Date(`2019-01-22 14:26:34`).toLocaleString(`en-US`,{year:`numeric`,month:`short`,day:`numeric`}),donor:`John Doe`,paymentMethod:`Credit Card`,status:`Completed`,id:`foo-id-4`,token:`foo-token-4`,is_test:!1}),new Qr({currency:`USD`,net_amount:100,total_amount:105.86,fee_amount:5.86,fee_covered:!0,date:new Date(`2022-09-22 14:26:34`).toLocaleString(`en-US`,{year:`numeric`,month:`short`,day:`numeric`}),donor:`John Doe`,paymentMethod:`Credit Card`,status:`Completed`,id:`foo-id-2`,token:`foo-token-2`,is_test:!0}),new Qr({currency:`USD`,net_amount:100,total_amount:222.88,fee_amount:122.88,date:new Date(`2023-12-23 14:26:34`).toLocaleString(`en-US`,{year:`numeric`,month:`short`,day:`numeric`}),donor:`John Doe`,paymentMethod:`Credit Card`,status:`Completed`,id:`foo-id-3`,token:`foo-token-3`,is_test:!1})],li={referrer:``,origin:``,braintreeAuthToken:`sandbox_x634jsj7_7zybks4ybp63pbmd`,venmoProfileId:`1953896702662410263`,googlePayMerchantId:``,environment:`dev`,paymentClients:new pn(new e,T.Development),endpointManager:{submitData:async e=>{},donationSuccessful:e=>{}}},ui=!0,Q=document.getElementById(`interaction-status-area`),di=()=>Math.floor(Math.random()+.5),fi=()=>document.getElementById(`force-successful-requests`).checked?1:di(),$=document.querySelector(`ia-monthly-giving-circle`);$.paymentConfig=li,$.receipts=ci,$.plans=si,$.addEventListener(`EmailReceiptRequest`,e=>{let{donation:t}=e.detail,n=fi()===1,r=n?`success`:`fail`,i=n?1500:5e3;Q.innerText=`Email receipt request for donation ${t.id} will return ${r} in ${i} ms.`;let a={message:r===`success`?`Email receipt sent`:`Email receipt failed`,status:r,donationId:t.id};Z=[a,...Z],setTimeout(()=>{$.updateReceived(a),console.log(`EmailReceiptRequest index.html ----`,a),Q.innerText=``},i)}),$.addEventListener(`cancelPlan`,async e=>{let{plan:t}=e.detail;$.plans=$.plans.map(e=>(e.plan.token===t.plan.token&&(console.log(`p 2 plan`,e,t),e.cancelPlan()),e)),await $.updateComplete,console.log(`plan cancelled & plan list updated`,$.plans)}),$.addEventListener(`updateAmount`,async e=>{let{plan:t,amountOptions:n}=e.detail,r=fi()===1,i=r?`success`:`fail`,a=r?1500:5e3;Q.innerText=`Amount Update Request received: will return ${i} in ${a} ms - Details: ${JSON.stringify(n)}`;let o=i===`success`?`Amount updated`:`Amount failed to update`;r&&t.setAmount(n.amount);let s={message:o,status:i,plan:t,donationId:t.id,action:`amountUpdate`};Z=[s,...Z],setTimeout(()=>{$.updateReceived(s),console.log(`Amount Update Request --- index.html ----`,s),Q.innerText=``},a)}),$.addEventListener(`updateDate`,e=>{let{newDate:t,plan:n}=e.detail,r=fi()===1,i=r?`success`:`fail`,a=r?1500:5e3;Q.innerText=`Updating next billing date for plan: ${t}, plan: ${JSON.stringify(n.id)} -- Update will return ${i} in ${a} ms`;let o=i===`success`?`Date updated`:`Date failed to update`;r&&(n.setNextBillingDate(t),n.setNewProcessorId(`new-processor_id@1`));let s={message:o,status:i,plan:n,donationId:n.id,action:`dateUpdate`};Z=[s,...Z],setTimeout(()=>{$.updateReceived(s),console.log(`Amount Update Request --- index.html ----`,s),Q.innerText=``},a)}),$.addEventListener(`UpdatePaymentMethod`,e=>{let{plan:t,newPaymentMethodRequest:n}=e.detail,r=fi()===1,i=r?`success`:`fail`,a=r?1500:4e3;Q.innerText=`Updating payment method for plan: ${t?t.id:`no plan`} -- Update will return ${i} in ${a} ms`;let o=i===`success`?`Payment method updated`:`Payment method failed to update`;r&&t&&(console.log(`demo - setting new payment method`,n),t.setNewPaymentMethod(n));let s={message:o,status:i,plan:t,donationId:t?t.id:null,action:`paymentMethodUpdate`};Z=[s,...Z],setTimeout(()=>{$.updateReceived(s),console.log(`Amount Update Request --- index.html ----`,s),Q.innerText=``},a)}),document.getElementById(`toggle-receipts`).addEventListener(`click`,async()=>{if(ui){$.receipts=[],ui=!1;return}$.receipts=ci,await $.updateComplete,ui=!0}),document.getElementById(`clear-updates`).addEventListener(`click`,async()=>{Z=[],$.updates=Z,await $.updateComplete}),document.getElementById(`toggle-plans`).addEventListener(`click`,async()=>{$.plans=$.plans.length?[]:si,await $.updateComplete}),document.getElementById(`toggle-prefilled-email`).addEventListener(`click`,async()=>{$.patronEmail=$.patronEmail?``:`you@thisemail.com`,await $.updateComplete}),document.getElementById(`toggle-edit-payment-method`).addEventListener(`click`,async()=>{$.canEditPaymentMethod=!$.canEditPaymentMethod,await $.updateComplete});